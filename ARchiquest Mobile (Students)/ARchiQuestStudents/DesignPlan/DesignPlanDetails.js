  ...
  useEffect(() => {
    if (designPlan) {
      ...
      if (designPlan.floor_area) {
        setFloorArea(parseFloat(designPlan.floor_area) || 0);
...
const fetchPlanDetails = async (planId) => {
  try {
    ...
    if (error) {
      ...
      // Set floor area from design plan if available
      if (data.floor_area) {
        setFloorArea(parseFloat(data.floor_area) || 0);
...
const fetchDesignImage = async (plan) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .storage
      .from('upload')
      .getPublicUrl(filePath); // use exact path from DB
...
// Add this to the useEffect where you fetch plan details
useEffect(() => {
  if (designPlan) {
    // If we already have the design plan data, use it
    setPlanDetails(designPlan);
  ...
  const handleSelectMaterial = (material, index) => {
    ...
    if (updatedMaterials[index].isSelected && updatedMaterials[index].quantity === 0) {
      ...
      if (material.unit.toLowerCase().includes('sqm') || 
  ...
  const saveEstimate = async () => {
    ...
    try {
      ...
      if (itemsError) {
        ...
        Alert.alert(
          "Success", 
          "Your cost estimate has been saved successfully!",
  ...
  const findAlternatives = (material) => {
    ...
    const similarMaterials = materials.filter(m => 
      m.id !== material.id && 
  ...
  const renderMaterialItem = ({ item, index }) => (
    <View style={[
      styles.materialItem, 
      item.isSelected && styles.selectedMaterialItem
    ]}>
      <View style={styles.materialHeader}>
        <TouchableOpacity 
          style={styles.materialSelectButton}
          onPress={() => handleSelectMaterial(item, index)}
        >
          <Ionicons 
        ...
        <View key={index} style={styles.tipItem}>
          ...
          <View style={styles.tipContent}>
            ...
            <Text style={styles.tipDescription}>
              Replace "{tip.material.material_name}" ({formatCurrency(tip.material.price, planDetails?.currency)}/{tip.material.unit}) 
              with "{tip.alternative.material_name}" ({formatCurrency(tip.alternative.price, planDetails?.currency)}/{tip.alternative.unit})
            </Text>
      ...
      return (
        <View key={index} style={styles.tipItem}>
          ...
          <View style={styles.tipContent}>
            ...
            <Text style={styles.tipDescription}>
              Reduce "{tip.material.material_name}" from {tip.material.quantity} to {tip.suggestedQuantity} {tip.material.unit}
            </Text>
            <Text style={styles.tipSavings}>
              Potential savings: {formatCurrency(tip.savings, planDetails?.currency)}
            </Text>
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          <View style={styles.modalHeader}>
            ...
            <TouchableOpacity 
              onPress={() => setIsEstimateModalVisible(false)}
          ...
          <Text style={styles.modalSubtitle}>
            Give your estimate a name to save it for future reference
          </Text>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.planHeader}>
            ...
            {floorArea > 0 && (
              <View style={styles.floorAreaBadge}>
                ...
                <Text style={styles.floorAreaText}>
          ...
          <View style={styles.fileSection}>
            ...
            {imageLoading ? (
              ...
              <TouchableOpacity onPress={() => setIsImageModalVisible(true)} style={{ alignItems: 'center', marginVertical: 12 }}>
                <Image
          ...
          <View style={styles.budgetCard}>
            ...
            <View style={styles.budgetStatusRow}>
              <View style={[styles.budgetStatusBadge, { backgroundColor: budgetStatus.color + '20', borderColor: budgetStatus.color }]}>
          ...
          <View style={styles.materialsSection}>
            <View style={styles.sectionTitleRow}>
              ...
              <Text style={styles.sectionSubtitle}>
                Select materials and specify quantities
              </Text>
            </View>
              ...
              <View style={styles.summaryTable}>
                ...
                {selectedMaterials.map((material, index) => (
                  <View key={index} style={styles.summaryTableRow}>
                    ...
                    <Text style={[styles.summaryTableCell, styles.summaryTableQty]}>
                      {material.quantity}
                    </Text>
                ...
                <View style={styles.summaryTableFooter}>
                  <Text style={[styles.summaryTableCell, styles.summaryTableMaterial]}>
          ...
          <View style={styles.actionButtons}>
            {selectedMaterials.length > 0 && (
              <TouchableOpacity 
                ...
                onPress={() => setIsEstimateModalVisible(true)}
              >
